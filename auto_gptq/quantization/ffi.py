...
class ComplieHelper(metaclass=SingletonMeta):
    ...
    def complie(self):
        ...
        lock_file = os.path.join(os.path.dirname(__file__), 'csrc/build/lock')
        ...
        self.__CUDA_EXTENTION__ = load(
            name='PPQ_Cuda_Impls',
            sources=[
                os.path.join(os.path.dirname(__file__), 'csrc/export.cc'),
                os.path.join(os.path.dirname(__file__), 'csrc/cuda/linear.cu'),
                os.path.join(os.path.dirname(__file__), 'csrc/cuda/sort.cu'),
                os.path.join(os.path.dirname(__file__), 'csrc/cuda/train.cu'),
                os.path.join(os.path.dirname(__file__), 'csrc/cuda/floating.cu'),
                os.path.join(os.path.dirname(__file__), 'csrc/cpu/hist_mse.cc'),
            ],
            build_directory=os.path.join(os.path.dirname(__file__), 'csrc/build/'),
            with_cuda=True,
            extra_cflags=['-O3'])
...
class CUDA:
    ...
    @ staticmethod
    def FloatingQuantize_T_Quant(
        # copied from FloatingQuantize_T, value -> value_quant
        tensor: torch.Tensor,
        scales: torch.Tensor,
        offsets: torch.Tensor,
        exponent: int = 4, 
        ...
        # if scale is too small, quantization might cause fp32 underflow.
        # if scale < 1e-7: raise ValueError('scale is too small.')
        return CUDA_COMPLIER.CUDA_EXTENSION.QuantizeTensor_FT_Quant(
            tensor, scales, offsets, exponent, mantissa, minimum, maximum, rounding)

    @ staticmethod
    def FloatingQuantize_T_Dequant(
        # copied from FloatingQuantize_T, value -> value_quant
        tensor: torch.Tensor,
        scales: torch.Tensor,
        offsets: torch.Tensor,
        ...
        # if scale is too small, quantization might cause fp32 underflow.
        # if scale < 1e-7: raise ValueError('scale is too small.')
        return CUDA_COMPLIER.CUDA_EXTENSION.QuantizeTensor_FT_Dequant(
            tensor, scales, offsets, exponent, mantissa, minimum, maximum, rounding)
