...
__global__ void _QuantizeTensor_FT(
    ...
    KERNEL_LOOP(iter, num_of_element){
        float qt = QuantizeScalarFloating<float, float, float>(
            __ldg(&value[iter]), __ldg(&scale[iter]), __ldg(&offset[iter]), exponent, mantissa, clip_min, clip_max, rounding);


        // out[iter] = qt;
        float deq = DequantizeScalar<float, float, float>(qt, __ldg(&scale[iter]), __ldg(&offset[iter]));
        out[iter] = deq;
    }
}

__global__ void _QuantizeTensor_FT_Quant(
    const int64_t    num_of_element,
    const float*     value,
    const float*     scale,
    const float*     offset,
    ...
    // copied from _QuantizeTensor_FT, value -> value_quant
    int64_t iter;
    KERNEL_LOOP(iter, num_of_element){
        float qt = QuantizeScalarFloating<float, float, float>(
            __ldg(&value[iter]), __ldg(&scale[iter]), __ldg(&offset[iter]), exponent, mantissa, clip_min, clip_max, rounding);
...
__global__ void _QuantizeTensor_FT_Dequant(
    const int64_t    num_of_element,
    const float*     value,
    const float*     scale,
    const float*     offset,
    ...
    // copied from _QuantizeTensor_FT, value_quant -> value

    int64_t iter;
    KERNEL_LOOP(iter, num_of_element){
        float deq = DequantizeScalar<float, float, float>(
            __ldg(&value[iter]), __ldg(&scale[iter]), __ldg(&offset[iter]));
        out[iter] = deq;
    }
}
...
__host__ Tensor QuantizeTensor_FT_Quant(
    const Tensor &value, const Tensor &scale, const Tensor &offset,
    const int exponent, const int mantissa,
    const float clip_min, const float clip_max, const Rounding rounding){
    // copied from QuantizeTensor_FT, value -> value_quant
    CheckTensor(value, at::kFloat, "Value(Expect to be FP32)");
    CheckTensor(scale, at::kFloat, "Scale(Expect to be FP32)");
    CheckTensor(offset, at::kFloat, "Offset(Expect to be FP32)");

    Tensor quantized = at::empty_like(value);
    _QuantizeTensor_FT_Quant<<<NUM_OF_BLOCK(NUM_OF_ELEMENT(value), CUDA_NUM_THREADS), CUDA_NUM_THREADS, 0, at::cuda::getCurrentCUDAStream()>>>(
        NUM_OF_ELEMENT(value), PTR<float>(value), PTR<float>(scale), PTR<float>(offset),
        exponent, mantissa, clip_min, clip_max, rounding, PTR<float>(quantized)
    );
    return quantized;
...
__host__ Tensor QuantizeTensor_FT_Dequant(
    const Tensor &value, const Tensor &scale, const Tensor &offset,
    const int exponent, const int mantissa,
    const float clip_min, const float clip_max, const Rounding rounding){
    /**
    ...
    Tensor dequantized = at::empty_like(value);
    _QuantizeTensor_FT_Dequant<<<NUM_OF_BLOCK(NUM_OF_ELEMENT(value), CUDA_NUM_THREADS), CUDA_NUM_THREADS, 0, at::cuda::getCurrentCUDAStream()>>>(
        NUM_OF_ELEMENT(value), PTR<float>(value), PTR<float>(scale), PTR<float>(offset),
        exponent, mantissa, clip_min, clip_max, rounding, PTR<float>(dequantized)
    );
    return dequantized;
}
