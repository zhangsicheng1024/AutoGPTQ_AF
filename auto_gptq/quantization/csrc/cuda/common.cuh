...
//     switch(rounding){
//         case ROUND_HALF_EVEN:
//             float num_round = std::nearbyint(value);
//             if (num_round == 0.0 && value < 0) return -0.5;
//             else if (num_round == 0.0 && value > 0) return 0.5;
//             else return num_round;
//             // return num_round;
//         // case ROUND_HALF_UP:
//         //     return floor(value + .5);
//         // case ROUND_HALF_DOWN:
...
float QuantizeScalarFloating(
    ...
    // float Unscaled_FP32 = static_cast<float>(value) / scale + offset;
	...
	if (((fp32_exp >> 23) - 127) < exponent_min + 1){
        ...
        // return _round2int(Unscaled_FP32 / 0.75, rounding) * 0.25f;
	}
